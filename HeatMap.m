...
% HeatMap MATLAB code for HeatMap.fig
%      HeatMap, by itself, creates a new HeatMap or raises the existing
%      singleton*.
%
%      H = HeatMap returns the handle to a new HeatMap or the handle to
%      the existing singleton*.
%
%      HeatMap('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in HeatMap.M with the given input arguments.
%
%      HeatMap('Property','Value',...) creates a new HeatMap or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before HeatMap_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to HeatMap_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help HeatMap

% Last Modified by GUIDE v2.5 20-Mar-2014 17:44:39

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @HeatMap_OpeningFcn, ...
                   'gui_OutputFcn',  @HeatMap_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before HeatMap is made visible.
function HeatMap_OpeningFcn(hObject, ~, handles, varargin)
...
function varargout = HeatMap_OutputFcn(~, ~, handles) 
